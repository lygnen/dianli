...
def train_and_predict():
    ...
    # 复制我们在 lgb_model.py 中使用的相同的参数范围 (这里取一个平均优良的经验值，或者直接用固定的较好参数，因为每次Optuna耗时较长)
    # 为了保证使用 lgb_model.py 的逻辑，我们也可以直接使用它里面的最佳参数逻辑，但这里为了预测，我们直接基于全量数据拟合
    models = {}
    ...
    output_file = 'LightGBM_预测结果_0320_0330.xlsx'
    ...
    plt.title('LightGBM 2026年3月20日至3月30日 电价预测曲线', fontsize=15)
    ...
    plt.tight_layout()
    plt.savefig('LightGBM_预测曲线_0320_0330.png', dpi=300)
    print("预测曲线已保存至 'LightGBM_预测曲线_0320_0330.png'")
